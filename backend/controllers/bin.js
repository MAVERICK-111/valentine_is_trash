...
const insertedBin = data[0];

// 📧 SEND MAIL ONLY IF BIN IS FULL
if (prediction === "no") {
  try {

    await sendCleaningAlert(insertedBin);
    console.log("✅ Cleaning alert sent");
  } catch (mailErr) {
    console.error("Mail failed:", mailErr.message);
  }
}

res.status(201).json(insertedBin);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
